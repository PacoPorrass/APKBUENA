...
class FileAdapter(
    ...
    inner class VH(private val b: ItemFileBinding) : RecyclerView.ViewHolder(b.root) {
            ...
            when {
                ...
                item.isFolder -> {
                    b.ivIcon.setImageResource(
                        if (item.isShared || item.driveId != null)
                    ...
                    b.tvChildCount.visibility =
                        if (item.childCount > 0) View.VISIBLE else View.GONE

                    b.tvChildCount.text = item.childCount.toString()
                }
                ...
                item.mimeType?.startsWith("image/") == true -> {
                    ...
                    b.tvChildCount.visibility = View.GONE
                ...
                // ─────────────────────────────
                    ...
                    b.ivIcon.setPadding(10, 10, 10, 10)
                    b.ivIcon.setImageResource(
                        iconForMime(item.mimeType, item.name)
                    )
                }
            }
        ...
        private fun buildMeta(item: FileItem): String {
            ...
            if (item.isShared || item.driveId != null) {
                parts.add("Compartido")
            }

            if (!item.isFolder && item.size > 0) {
                parts.add(formatSize(item.size))
            }

            if (item.lastModified.isNotBlank()) {
                parts.add(formatDate(item.lastModified))
            }
        ...
        private fun iconForMime(mime: String?, name: String) = when {
            ...
            mime?.contains("word") == true ||
                    name.endsWith(".doc") ||
                    name.endsWith(".docx") -> R.drawable.ic_doc

            mime?.contains("excel") == true ||
                    name.endsWith(".xls") ||
                    name.endsWith(".xlsx") -> R.drawable.ic_xls
