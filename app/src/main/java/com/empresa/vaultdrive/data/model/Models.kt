...
data class RemoteItem(@SerializedName("id") val id: String? = null, @SerializedName("parentReference") val parentReference: ParentRef? = null)
...
data class FileItem(val id: String, val name: String, val isFolder: Boolean, val size: Long = 0L, val lastModified: String = "", val mimeType: String? = null, val downloadUrl: String? = null, val childCount: Int = 0, val isShared: Boolean = false)
fun DriveItem.toFileItem() = FileItem(id = id, name = name, isFolder = isFolder, size = size ?: 0L, lastModified = lastModifiedDateTime ?: "", mimeType = file?.mimeType, downloadUrl = downloadUrl, childCount = folder?.childCount ?: 0, isShared = remoteItem != null)
