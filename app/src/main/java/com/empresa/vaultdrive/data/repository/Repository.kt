...
class Repository(private val context: Context) {
    ...
    suspend fun getChildren(
        ...
        try {
            val resp = if (!driveId.isNullOrBlank()) {
                ...
                api.getChildren(bearer(token), folderId)
            }

            if (!resp.isSuccessful) {
                return@withContext Result.Error("Error ${resp.code()}: ${resp.message()}")
            }
    ...
    // COMPARTIDOS
    // ─────────────────────────────────────────────
    suspend fun getSharedWithMe(token: String): Result<List<FileItem>> =
    ...
    suspend fun openSharedLink(token: String, url: String): Result<FileItem> =
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                if (!resp.isSuccessful) {
                    return@withContext Result.Error("Error ${resp.code()}")
                }

                Result.Success(resp.body()!!.toFileItem())

            } catch (e: Exception) {
                Result.Error(e.message ?: "Error link compartido")
            }
        }
    ...
    suspend fun createFolder(
        ...
        try {
            val resp = api.createFolder(
                bearer(token),
                parentId,
                CreateFolderRequest(name = name.trim())
            )

            if (resp.isSuccessful)
                Result.Success(resp.body()!!.toFileItem())
            else
                Result.Error("Error ${resp.code()}")
    ...
    suspend fun uploadFile(
        ...
        try {
            ...
            } ?: return@withContext Result.Error("No se puede abrir archivo")

            if (tmp.length() <= 4 * 1024 * 1024) {

                onProgress(10)

                val resp = if (driveId != null) {
                    api.uploadSmallShared(
                        bearer(token),
                        driveId,
                        parentId,
                        finalName,
                        tmp.asRequestBody(getMime(finalName).toMediaType())
                    )
                } else {
                    api.uploadSmall(
                        bearer(token),
                ...
                uploadLarge(token, parentId, finalName, tmp, driveId, onProgress)
            }

        } catch (e: Exception) {
            Result.Error(e.message ?: "Error upload")
        } finally {
            tmp?.delete()
        }
    }
    ...
    private suspend fun uploadLarge(
        ...
        val sessionResp = api.createUploadSession(
            bearer(token),
            parentId,
            name,
        ...
        file.inputStream().use { stream ->
            while (offset < fileSize) {
                ...
                if (!resp.isSuccessful && resp.code != 202) {
                    return Result.Error("Error chunk ${resp.code}")
                }

                offset += len
                onProgress(((offset.toFloat() / fileSize) * 100).toInt())
            }
        }
    ...
    private fun getMime(name: String) =
